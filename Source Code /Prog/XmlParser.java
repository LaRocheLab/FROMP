...
public class XmlParser {
    ...
    public ArrayList<EcPosAndSize> findEcPosAndSize(String ecNr, BufferedReader in) { // finds
										      // the
										      // positions
										      // and
										      // sizes
	...
	try {
		...
		if (zeile.contains("entry id=\"") && zeile.contains(ecNr)) {
		    if (((zeile = in.readLine()) != null) && (zeile.contains("link=\""))) {
			tmp = "link=\"";
		...
		if (zeile.contains("graphics name=\"" + ecNr + "\"")) {
		    if (((zeile = in.readLine()) != null) && (zeile.contains("type=\"rectangle\""))) {
			tmp = "x=\"";
			begin = zeile.indexOf(tmp) + tmp.length();
			end = zeile.indexOf("\"", begin);
			tmpNr = zeile.substring(begin, end);
			...
			EcPosAndSize tmpPos = new EcPosAndSize(tmpX, tmpY, tmpH, tmpW, tmpUrl);
			ret.add(tmpPos);
		    } else if (zeile.contains("coords")) {// adding read
								 // line
								 // function
			tmp = "coords=\"";
			begin = zeile.indexOf(tmp) + tmp.length();
			end = zeile.lastIndexOf("\"");
			tmpCoords = zeile.substring(begin, end);
			String[] coords = tmpCoords.split(",");
			EcPosAndSize tmpPos = new EcPosAndSize(tmpUrl,coords);
			ret.add(tmpPos);
    ...
    public void findEcinPathway(String workpath, String[] ecNr, BufferedReader in, String name) {
	String zeile = null;
	...
	try {
	    ...
	    out = new BufferedWriter(new FileWriter(workpath + "/output/" + this.date + "/" + name));
	} catch (IOException e1) {
	    e1.printStackTrace();
	}
	try {
		...
		for (int i = 0; (i < ecNr.length) && (ecNr[i].length() > 0); i++) {
		    if (!ecNr[i].isEmpty()) {
			if (zeile.contains("graphics name=\"" + ecNr[i].subSequence(3, ecNr[i].length()) + "\"")) {
			    String newLine = null;

			    offset = zeile.indexOf("fgcolorwindows 10 upgrade icon now showing=") + 9;
			    newLine = zeile.substring(0, offset) + "#FF0000" + zeile.substring(offset + 7);
