...
public class NewFrompFrame extends JFrame {
    ...
    private JMenuBar menuBar_; // Menu bar at the top of Fromp. ie 'File'
    // ,'Project' , 'Analyze' etc.
    private JMenu menu_;
    private JPanel back_; // The back panel on this JFrame
    public static ArrayList<String> recentProj_;
    /*
     * The base path of the Fromp software nessesary for all relative paths to
     * function
     */
    final String basePath_ = new File(".").getAbsolutePath() + File.separator;
    final String recentProjPath_ = "recentProj.txt"; // Path to the file which
    // contains the recently
    // opened projects
    private Controller control_; // The controller. Allows user to save, load
    // etc.
    private JScrollPane showJPanel_; // Used only to be able to scroll through
    // samples in the EditSamplesPane
	...
	addWindowListener(new WindowListener() {
		...
		// openClosingFrame() is a warning window, for reminding to save
		// current work
	...
	showNewProjPanel(); // Shows the starting panel of Fromp. Includes
	// 'Project Options' , and 'Recent Projects'
	invalidate(); // Next three commands in combination reload the JFrame
	validate();
	repaint();
    }

    private void addMenu() {// adds the drop down menu bar and all of its
	// elements
	...
	this.menu_.getAccessibleContext().setAccessibleDescription("The only menu in this program that has menu items");
	...
	miItem.getAccessibleContext().setAccessibleDescription("Open a project");
	miItem.addActionListener(new ActionListener() {
	    public void actionPerformed(ActionEvent e) {
	...
	miItem.getAccessibleContext().setAccessibleDescription("Export Project as");
	miItem.addActionListener(new ActionListener() {
	    public void actionPerformed(ActionEvent e) {
	...
	this.menu_.getAccessibleContext().setAccessibleDescription("Manage your Project");
	this.menuBar_.add(this.menu_);

	JMenuItem mItem = new JMenuItem("Edit Samples", 68);
	mItem.setAccelerator(KeyStroke.getKeyStroke(68, 8));
	mItem.getAccessibleContext().setAccessibleDescription("Add Samples to your project");
	...
	mItem.getAccessibleContext().setAccessibleDescription("Select wanted Pathways");
	mItem.addActionListener(new ActionListener() {
	    public void actionPerformed(ActionEvent e) {
	...
	mItem.getAccessibleContext().setAccessibleDescription("Pathway completeness score");
	mItem.addActionListener(new ActionListener() {
	    public void actionPerformed(ActionEvent e) {
		if (NewFrompFrame.this.control_.gotSamples()) {
		    NewFrompFrame.this.clearBack();
	...
	mItem.getAccessibleContext().setAccessibleDescription("EC Activity Matrix");
	mItem.addActionListener(new ActionListener() {
	    public void actionPerformed(ActionEvent e) {
		if (NewFrompFrame.this.control_.gotSamples()) {
		    NewFrompFrame.this.clearBack();
	...
	mItem.getAccessibleContext().setAccessibleDescription("Lowest Common Ancestor");
	mItem.addActionListener(new ActionListener() {
	    public void actionPerformed(ActionEvent e) {
		if (NewFrompFrame.this.control_.gotSamples()) {
		    NewFrompFrame.this.clearBack();
	...
	mItem.getAccessibleContext().setAccessibleDescription("Set Background Color");
	mItem.addActionListener(new ActionListener() {
	    public void actionPerformed(ActionEvent e) {
		...
		NewFrompFrame.this.back_.setBackground(NewFrompFrame.this.backCol_);
		// NewFrompFrame.this.clearBack();
	...
	mItem.setAccelerator(KeyStroke.getKeyStroke(70, 8));
	mItem.getAccessibleContext().setAccessibleDescription("Set System Color");
	mItem.addActionListener(new ActionListener() {
	    public void actionPerformed(ActionEvent e) {
	...
	mItem.setAccelerator(KeyStroke.getKeyStroke(67, 8));
	mItem.getAccessibleContext().setAccessibleDescription("Set Overall Sample Color");
	mItem.addActionListener(new ActionListener() {
	    public void actionPerformed(ActionEvent e) {
	...
	mItem.addActionListener(new ActionListener() {
	    public void actionPerformed(ActionEvent e) {
		String file = NewFrompFrame.this.basePath_.substring(0, NewFrompFrame.this.basePath_.length() - 2)
	...
	if (Project.workpath_ != null) {
	    setTitle("Project: " + Project.workpath_ + " - FROMP-Alpha - Fragment Recruitment on Metabolic Pathways");
	} else {
	    setTitle("FROMP-Alpha - Fragment Recruitment on Metabolic Pathways");
	}
	invalidate();
	...
	// analysis
	clearBack();
	PathwaysPane pwPanes = new PathwaysPane(Controller.project_, Controller.processor_, getSize());
	...
	clearBack();
	PathwayActPanes actP = new PathwayActPanes(Controller.project_, Controller.processor_, getSize());
	...
	clearBack();
	EcActPanes matrixP_ = new EcActPanes(Controller.project_, Controller.processor_, getSize());
	...
	clearBack();
	GoActPanes matrixP_ = new GoActPanes(Controller.project_, Controller.processor_, getSize());
	matrixP_.backButton_.addActionListener(new ActionListener() {
	    public void actionPerformed(ActionEvent e) {
		NewFrompFrame.this.clearBack();
		NewFrompFrame.this.showAnalyseOptions();
	...
	PwSearchPane pwSearch = new PwSearchPane(Controller.project_, Project.samples_, Project.overall_, getSize());
    ...
    // read recent project list from recentProj.txt, then save it into arrayList
    // recentProj_
    private void loadRecentProj() {
	try {
	    BufferedReader in = new BufferedReader(new FileReader(StartFromp1.FolderPath + "recentProj.txt"));
	...
	try {
	    BufferedWriter out = new BufferedWriter(new FileWriter(StartFromp1.FolderPath + "recentProj.txt"));
	...
	// clearBack();
	JFileChooser fChoose_ = new JFileChooser(this.path_ + File.separator + "projects");
	...
	fChoose_.setFileFilter(new FileFilter() {
		...
		if ((f.isDirectory()) || (f.getName().toLowerCase().endsWith(".prj"))
	...
	if (fChoose_.showOpenDialog(getParent()) == 0) {
	    try {
		int erg = this.control_.loadProjFile(fChoose_.getSelectedFile().getCanonicalPath());
	...
	fChoose_.setFileFilter(new FileFilter() {
	    public boolean accept(File f) {
		if ((f.isDirectory()) || (f.getName().toLowerCase().endsWith(".frp"))) {
		    return true;
		}
		return false;
	    }
	...
	if (Project.samples_.size() == 0) {
	    ...
	    JLabel label = new JLabel("Warning all unsaved progress will be lost!");
