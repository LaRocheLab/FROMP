...
public class PngBuilder {
    ...
    public BufferedImage getAlteredPathway(ArrayList<EcPosAndSize> posList, String pathwayId, Sample sample) {
	System.out.println("1");
	Color col = sample.sampleCol_;
	try {
	    this.image = ImageIO.read(new File(StartFromp1.FolderPath + "pics" + this.separator_ + pathwayId + ".png"));
    ...
    public void alterPathway(ArrayList<EcPosAndSize> posList, String pathwayId, Sample sample) {
	Color col = sample.sampleCol_;
	try {
	    this.image = ImageIO.read(new File(StartFromp1.FolderPath + "pics" + this.separator_ + pathwayId + ".png"));
	    reColorAllEcs(this.image, Color.white);
	...
	try {
	    File outputfile = new File("outpics" + this.separator_ + pathwayId + ".png");
	...
	try {
	    this.image = ImageIO
		    .read(new File(StartFromp1.FolderPath + "pics" + this.separator_ + tmpPath.id_ + ".png"));
	    reColorAllEcs(this.image, Color.white);
		...
		for (int i = 0; i < tmpEc.posSize_.size(); i++) {
		    ...
		    stepper = 0;
		    for (int x = xStart; x < xStart + tmpPos.width_; x++) {
			if ((!sample.singleSample_) && (stepper > tmpStats.amount_ * sampSteps)
	...
	try {
	    File outputfile = new File("outpics" + this.separator_ + tmpPath.id_ + ".png");
	    ImageIO.write(this.image, "png", outputfile);
	} catch (IOException localIOException1) {
	}
    }
	...
	try {
	    this.image = ImageIO
		    .read(new File(StartFromp1.FolderPath + "pics" + this.separator_ + tmpPath.id_ + ".png"));
	    if (!tmpPath.id_.contentEquals("ec01100") && !tmpPath.id_.contentEquals("ec01110")
		    && !tmpPath.id_.contentEquals("ec01120")) {
		reColorAllEcs(this.image, Color.white);
	    }
		...
		for (int i = 0; i < tmpEc.posSize_.size(); i++) {
		    ...
		    // making the colored portions of the pathway picture click
		    // able in order to open the ec numbers web page
		    ...
		    labelList.add(label);
		    if (tmpPos.coords != null) {
			int x1 = Integer.parseInt(tmpPos.coords[0]);
			int y1 = Integer.parseInt(tmpPos.coords[1]);
			int x2 = Integer.parseInt(tmpPos.coords[2]);
			int y2 = Integer.parseInt(tmpPos.coords[3]);
			xStart = (x1 + x2) / 2 - 40;
			yStart = (y1 + y2) / 2;
			tmpPos.width_ = 60;
			tmpPos.height_ = 15;
			label.setText(tmpEc.name_);

		    }
		    label.setBounds(xStart, yStart + 50, tmpPos.width_, tmpPos.height_);

		    // if the pathway contains multiple instances of the same EC
		    // must be added to an arrayList of labels
		    ...
		    if ((!sample.singleSample_) && (statsCnt < tmpEc.stats_.size())) {
			tmpStats = (EcSampleStats) tmpEc.stats_.get(statsCnt);
			col = tmpStats.col_;
			stepper = 0;
		    ...
		    for (int x = xStart; x < xStart + tmpPos.width_; x++) {
			if ((!sample.singleSample_) && (stepper > tmpPos.width_ * perc)) {
	...
	for (int x = 0; x < image.getWidth(); x++) {
		...
		if ((image.getRGB(x, y) != Color.black.getRGB()) && (image.getRGB(x, y) != Color.white.getRGB())) {
